...
void main(){ vec2 uv=gl_FragCoord.xy/uResolution;\n\
  ...
  function run(ctn){
   ...
   function init(){ /* Perf: skip WebGL aurora on small/touch screens — static hero there. */
     if (g.matchMedia && g.matchMedia('(max-width: 760px)').matches) return;
     var els=document.querySelectorAll('.aurora'); for(var i=0;i<els.length;i++) if(!els[i].__a){ els[i].__a=1; run(els[i]); } }
